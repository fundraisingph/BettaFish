"""
Unified OpenAI-compatible LLM client for Insight Engine, with retry support.
"""
...
from ..utils.config import InsightEngineSettings as settings


class LLMClient:
    """Minimal wrapper around OpenAI-compatible chat completion API."""

    def __init__(self, api_key: str, model_name: str, base_url: Optional[str] = None):
        if not api_key:
            raise ValueError("Insight Engine API key is required.")
        if not model_name:
    ...
    def stream_invoke(self, system_prompt: str, user_prompt: str, **kwargs) -> Generator[str, None, None]:
        """
        Stream LLM invocation, yielding response chunks
    ...
    def stream_invoke_to_string(self, system_prompt: str, user_prompt: str, **kwargs) -> str:
        """
        Stream LLM invocation and safely concatenate to complete string
        ...
        """
        # Collect all chunks as bytes to avoid UTF-8 multi-byte character truncation
        byte_chunks = []
        for chunk in self.stream_invoke(system_prompt, user_prompt, **kwargs):
            byte_chunks.append(chunk.encode('utf-8'))
        
        # Concatenate all bytes, then decode once
        if byte_chunks:
