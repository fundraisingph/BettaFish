"""
Multilingual sentiment analysis tool for InsightEngine
"""
...
# INFO: If you want to skip sentiment analysis, manually switch this switch to False
...
# Add project root directory to path for WeiboMultilingualSentiment import
...
class SentimentAnalyzer:
    ...
    Wraps WeiboMultilingualSentiment model to provide sentiment analysis functionality for AI Agent
    """
    ...
    def __init__(self):
        ...
        if not SENTIMENT_ANALYSIS_ENABLED:
            self.disable("Sentiment analysis functionality has been disabled in configuration.")
        elif not (TORCH_AVAILABLE and TRANSFORMERS_AVAILABLE):
            missing = _describe_missing_dependencies() or "Unknown dependencies"
            self.disable(f"Missing dependencies: {missing}, sentiment analysis has been disabled.")

        if self.is_disabled:
            reason = self.disable_reason or "Sentiment analysis disabled."
            logger.warning(f"WeiboMultilingualSentimentAnalyzer initialized but disabled: {reason}")
        else:
            logger.info("WeiboMultilingualSentimentAnalyzer created, call initialize() to load model")
    ...
    async def initialize(self) -> bool:
        ...
        """
        if self.is_disabled:
            reason = self.disable_reason or "Sentiment analysis functionality disabled"
            logger.warning(f"Sentiment analysis functionality disabled, skipping model loading: {reason}")
            return False

        if not (TORCH_AVAILABLE and TRANSFORMERS_AVAILABLE):
            missing = _describe_missing_dependencies() or "Unknown dependencies"
            self.disable(f"Missing dependencies: {missing}, sentiment analysis has been disabled.", drop_state=True)
            logger.warning(f"Missing dependencies: {missing}, unable to load sentiment analysis model.")
            return False

        if self.is_initialized:
            logger.info("Model already initialized, no need to reload")
            return True
        ...
        try:
            ...
            if os.path.exists(local_model_path):
                ...
                logger.info("First use, downloading model to local...")
            ...
            if device_type == "cuda":
                logger.info("GPU detected, CUDA will be prioritized for inference.")
            elif device_type == "mps":
                logger.info("Apple MPS device detected, MPS will be used for inference.")
            else:
                logger.info("No GPU detected, CPU will be used for inference.")

            logger.info(f"Model loaded successfully! Using device: {self.device}")
            logger.info("Supported languages: Chinese, English, Spanish, Arabic, Japanese, Korean, etc. 22 languages")
            logger.info("Sentiment levels: Very Negative, Negative, Neutral, Positive, Very Positive")
            ...
            logger.error("Please check network connection or model files")
    ...
    async def analyze_single_text(self, text: str) -> SentimentResult:
        """
        ...
        if self.is_disabled:
            return SentimentResult(
                ...
                success=False,
                error_message=self.disable_reason or "Sentiment analysis functionality disabled",
        ...
        try:
            ...
            # Construct probability distribution dictionary
            prob_dist = {}
    ...
    async def analyze_batch(
        ...
        if self.is_disabled or not self.is_initialized:
            passthrough_results = [
                SentimentResult(
                    ...
                    probability_distribution={},
                    success=False,
                    error_message=self.disable_reason or "Sentiment analysis functionality unavailable",
                    analysis_performed=False,
                )
                for text in texts
            ]
        ...
        for i, text in enumerate(texts):
            ...
            result = await self.analyze_single_text(text)
    ...
    async def analyze_query_results(
        ...
        Specialized for analyzing query results returned from MediaCrawlerDB
        
        Args:
            query_results: Query result list, each element containing text content
            text_field: Text content field name, default is "content"
        ...
        logger.info(f"Performing sentiment analysis on {len(texts_to_analyze)} pieces of content...")
        batch_result = await self.analyze_batch(texts_to_analyze, show_progress=True)

        if not batch_result.analysis_performed:
            reason = self.disable_reason or "Sentiment analysis functionality unavailable"
        ...
        if total_analyzed > 0:
            ...
            sentiment_summary = f"Analyzed {total_analyzed} pieces of content, main sentiment tendency is '{dominant_sentiment[0]}'({dominant_sentiment[1]} pieces, accounting for {dominant_sentiment[1] / total_analyzed * 100:.1f}%)"
        else:
...
multilingual_sentiment_analyzer = SentimentAnalyzer()


async def enable_sentiment_analysis() -> bool:
    """Public helper to enable sentiment analysis at runtime"""
    return await multilingual_sentiment_analyzer.enable()
...
async def analyze_sentiment(
    ...
    Args:
        ...
        initialize_if_needed: Whether to automatically initialize if model is not initialized
    ...
    if (
        ...
        await multilingual_sentiment_analyzer.initialize()

    if isinstance(text_or_texts, str):
        return await multilingual_sentiment_analyzer.analyze_single_text(text_or_texts)
    else:
        texts_list = list(text_or_texts)
        return await multilingual_sentiment_analyzer.analyze_batch(texts_list)