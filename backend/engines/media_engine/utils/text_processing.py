...
def remove_reasoning_from_output(text: str) -> str:
    ...
    # If no JSON markers found, try other methods
...
def fix_incomplete_json(text: str) -> str:
    ...
    Returns:
        Fixed JSON text, return empty string if cannot be fixed
    """
    ...
    # Check if missing opening array symbol
    if text.strip().startswith('{') and not text.strip().startswith('['):
        # If starts with object, try to wrap into array
        if text.count('{') > 1:
            # Multiple objects, wrap into array
            text = '[' + text + ']'
        else:
            # Single object, wrap into array
            text = '[' + text + ']'
    
    # Check if missing closing array symbol
    if text.strip().endswith('}') and not text.strip().endswith(']'):
        # If ends with object, try to wrap into array
        if text.count('}') > 1:
            # Multiple objects, wrap into array
            text = '[' + text + ']'
        else:
...
def fix_aggressive_json(text: str) -> str:
    """
    More aggressive JSON fixing method
    ...
    if len(objects) >= 2:
        # If multiple objects, wrap into array
        return '[' + ','.join(objects) + ']'
    elif len(objects) == 1:
        # If only one object, wrap into array
        return '[' + objects[0] + ']'
    else:
...
def update_state_with_search_results(search_results: List[Dict[str, Any]],
    ...
    if 0 <= paragraph_index < len(state.paragraphs):
        # Get last search query (assuming it's current query)
        current_query = ""
        if search_results:
            # Infer query from search results (this needs improvement to get actual query)
